// This example is adapted from the original CPU openGJK repository example in order to match their established API.
// It is used to test the GPU implementation of the GJK algorithm.
// Here is the original liscense information:
//                           _____      _ _  __                                   //
//                          / ____|    | | |/ /                                   //
...
readinput(const char* inputfile, gkFloat** pts, int* out) {
  ...
  /* Allocate memory for flattened array (npoints * 3 floats). */
  ...
  /* Read and store vertices' coordinates in flattened format. */
...
main() {
  ...
  /* Compute squared distance using GJK algorithm on GPU. */
  /* Note: GPU API takes arrays, so we pass arrays with n=1 */
  gkFloat distances[1];
  gkSimplex simplices[1] = {s};
  simplices[0].nvrtx = 0;
  
  GJK::GPU::computeDistances(1, &bd1, &bd2, simplices, distances);
